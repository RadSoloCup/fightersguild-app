...
const api = {
  ...
  // Updater — checks GitHub Releases; "download"/"install" open the release page
  onUpdaterEvent: cb => on('updater-event', cb),
  updaterCheck: ctx => ipcRenderer.invoke('updater-check', ctx),
  updaterDownload: ctx => ipcRenderer.invoke('updater-download', ctx),
  ...
  // Switch to the Ops Center tab (Forum / Events / Servers)
  openPortal: () => ipcRenderer.send('tab-switch', 'ops'),
  switchTab: name => ipcRenderer.send('tab-switch', name),
  ...
  getDesktopSources: (types, requestId, options) =>
    ipcRenderer.invoke('get-desktop-sources', types, requestId, options),
  onDisplayMediaRequested: cb => on('display-media-requested', cb),
  onDisplayMediaPortalEmpty: cb => on('display-media-portal-empty', cb),
...
// ── Voice / mute state → tray icon ──────────────────────────────────────────
// Fluxer's web app does not report mic state to the desktop shell, so we read
// it from the DOM. If Fluxer's markup changes and the tray colour stops
// tracking, adjust the regexes below (open DevTools with F12 and check what
// aria-labels the voice controls actually use).
function readVoiceState() {
  let labels
  try {
    labels = Array.from(document.querySelectorAll('button, [role="button"], [aria-label], [title]'))
      .map(el => `${el.getAttribute('aria-label') || ''} ${el.getAttribute('title') || ''}`.trim())
      .filter(Boolean)
  } catch { return { inVoice: false, muted: false } }
  const has = re => labels.some(t => re.test(t))
  // "Disconnect" / "Leave voice" only exists while connected to a voice channel.
  const inVoice = has(/disconnect|leave\s*(voice|call)|hang\s*up/i)
  if (!inVoice) return { inVoice: false, muted: false }
  // The mic toggle reads "Unmute" while you are muted, "Mute" while live.
  return { inVoice: true, muted: has(/\bunmute\b/i) }
}
...
function pushVoiceState() {
  try {
    ...
    const key = `${s.inVoice}:${s.muted}`
    if (key === _lastVoiceKey) return
    _lastVoiceKey = key
    ipcRenderer.send('voice-state', s)
  } catch {}
}
...
function startVoiceWatch() {
  ...
  const obs = new MutationObserver(() => {
    clearTimeout(t)
    t = setTimeout(pushVoiceState, 250)
  })
...
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', startVoiceWatch, { once: true })
} else {
  startVoiceWatch()
}

// Forward voice-diagnostics lines from the injected main-world script (see
// main.js injectVoiceDiag) over IPC. CustomEvents cross the isolated/main
// world boundary because the DOM is shared.
window.addEventListener('fg-voice-log', e => {
  try {
    if (e && typeof e.detail === 'string') ipcRenderer.send('voice-log', e.detail.slice(0, 2000))
  } catch {}
})
